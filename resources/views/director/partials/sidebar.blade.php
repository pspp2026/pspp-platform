<aside
    ...
    <div class="flex items-center justify-between border-b border-slate-800 px-5 py-5">
        <a href="{{ route('director.dashboard') }}" class="flex items-center gap-3">
    ...
    <nav class="flex-1 space-y-7 overflow-y-auto px-4 pb-6">
        ...
        <div>
            ...
            <a href="{{ route('director.dashboard') }}"
               ...
               {{ request()->routeIs('director.dashboard') ? 'bg-blue-600 text-white shadow-lg' : 'hover:bg-purple-800 hover:translate-x-1' }}">
        ...
        <div>
            ...
            <div class="space-y-2">
                <a href="{{ route('director.subjects.index') }}"
                   @click="sidebarOpen = false"
                   class="group flex items-center gap-3 rounded-xl px-4 py-3 transition-all duration-200
                   {{ request()->routeIs('director.subjects.*') ? 'bg-blue-600 text-white shadow' : 'hover:bg-purple-800 hover:translate-x-1' }}">
                    <span class="text-lg">📚</span>
                    <span>รายวิชา</span>
                </a>

                <a href="{{ route('director.classrooms.index') }}"
                   @click="sidebarOpen = false"
                   class="group flex items-center gap-3 rounded-xl px-4 py-3 transition-all duration-200
                   {{ request()->routeIs('director.classrooms.*') ? 'bg-blue-600 text-white shadow' : 'hover:bg-purple-800 hover:translate-x-1' }}">
                    <span class="text-lg">🏫</span>
                    <span>ห้องเรียน</span>
                </a>

                <a href="{{ route('director.schedules.index') }}"
                   @click="sidebarOpen = false"
                   class="group flex items-center gap-3 rounded-xl px-4 py-3 transition-all duration-200
                   {{ request()->routeIs('director.schedules.*') ? 'bg-blue-600 text-white shadow' : 'hover:bg-purple-800 hover:translate-x-1' }}">
                    <span class="text-lg">📅</span>
                    <span>ตารางสอน</span>
                </a>
                ...
                <a href="{{ route('director.students.index') }}"
                   @click="sidebarOpen = false"
                   class="group flex items-center gap-3 rounded-xl px-4 py-3 transition-all duration-200
                   {{ request()->routeIs('director.students.*') ? 'bg-blue-600 text-white shadow' : 'hover:bg-purple-800 hover:translate-x-1' }}">
                    <span class="text-lg">🎓</span>
                    <span>รายชื่อนักเรียน</span>
                </a>

                <a href="{{ route('director.enrollments.import') }}"
                   @click="sidebarOpen = false"
                   class="group flex items-center gap-3 rounded-xl px-4 py-3 transition-all duration-200
                   {{ request()->routeIs('director.enrollments.import') ? 'bg-blue-600 text-white shadow' : 'hover:bg-purple-800 hover:translate-x-1' }}">
                    <span class="text-lg">📥</span>
                    <span>Import นักเรียน</span>
                </a>

                <a href="{{ route('director.enrollments.index') }}"
                   @click="sidebarOpen = false"
                   class="group flex items-center gap-3 rounded-xl px-4 py-3 transition-all duration-200
                   {{ request()->routeIs('director.enrollments.*') ? 'bg-blue-600 text-white shadow' : 'hover:bg-purple-800 hover:translate-x-1' }}">
                    <span class="text-lg">🎓</span>
